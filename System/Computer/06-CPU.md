...
硬件系统的核心是中央处理器（Central Processing Unit，简称 CPU）。它主要由寄存器、控制器、运算器和时钟组成，并采用大规模**集成电路**(Integrated Circuit, IC)工艺制成的芯片，又称微处理器芯片。程序是由指令构成的, 处理器是执行指令的硬件设备, 一个系统中可能有多个处理器.
